...
10. ✅ **Push a GitHub (opción 1 del usuario):** historia local reescrita (rama orphan) para eliminar los binarios del repo; `bin/` agregado a `.gitignore`. `package.json` → **4.0.0**. Push force a `origin/main` (`3a1b5a5`) sin binarios: GitHub lo aceptó (verificado: `origin/main` no contiene `bin/`).
11. ✅ **Workflow CI subido y verificado:** `.github/workflows/release.yml` pusheado; tag `v4.0.0` disparó el build. Release `v4.0.0` publicado con `AulaInicial-linux` (118 MB) y `AulaInicial.exe` (101 MB) en GitHub Releases. Los docentes los descargan de ahí.
12. 🔜 Mover frontend a `public/` y dividir `admin.js`/`index.js` en ES modules (pendiente, no bloquea el push).
13. 🔐 Revocar los tokens de GitHub (PAT) expuestos en el chat y generar uno nuevo con alcance `repo`+`workflow` para futuros pushes/CI.
